...
class FillintheblanksValidationException implements Exception {
  final List<String> errors;

  FillintheblanksValidationException(this.errors);

  @override
  String toString() => 'FillintheblanksValidationException: ${errors.join("; ")}';
}
...
/// Fill in the Blanks Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/fillintheblanks?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [category] - Puzzle category
/// * [count] - Number of puzzles [min: 1, max: 20]
/// * [difficulty] - Difficulty level
class FillintheblanksClient {
  final String apiKey;
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw FillintheblanksValidationException(errors);
    }
  }

  /// Execute a request to the Fill in the Blanks Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<FillintheblanksResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw FillintheblanksException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return FillintheblanksResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw FillintheblanksException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw FillintheblanksException('Resource not found');
      } else {
        throw FillintheblanksException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is FillintheblanksException) rethrow;
      throw FillintheblanksException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Fill in the Blanks Generator API client.
class FillintheblanksException implements Exception {
  final String message;

  FillintheblanksException(this.message);

  @override
  String toString() => 'FillintheblanksException: $message';
}
